class triggerSpawner
{
    file = "components\triggerSpawner";
    class activateTriggerSpawner{};
    class activateTriggerSpawnerNode{};
    class configureTriggerSpawner{};
    class spawnConfigEntry{};
    class triggerSpawner_giveOrders{};
    class validateTriggerSpawnerConfigMap{};
};

class triggerSpawner_orders
{
    file = "components\triggerSpawner\orders";
    class orders_garrisonArea{};
    class orders_garrisonBuilding{};
    class orders_noOrders{};
    class orders_patrolArea{};
    class orders_taskHunt{};
    class orders_taskRush{};
};