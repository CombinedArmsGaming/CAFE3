...
case "ui_init": {
	...
	private _setupNumericalSetting = {
		...
		_ctrlSlider ctrlAddEventHandler ["MouseButtonDown", {["ui_slider_start", _this] call f_fnc_zeusUI}];
		_ctrlSlider ctrlAddEventHandler ["MouseMoving", {["ui_slider_move", _this] call f_fnc_zeusUI}];
		_ctrlSlider ctrlAddEventHandler ["MouseButtonUp", {["ui_slider_stop", _this] call f_fnc_zeusUI}];

		// Add the textbox event handlers
		_ctrlTextBox ctrlAddEventHandler ["KeyDown", {["ui_textbox_changed", _this] call f_fnc_zeusUI}];
		_ctrlTextBox ctrlAddEventHandler ["MouseButtonDown", {["ui_textbox_clear", _this] call f_fnc_zeusUI}];
	};
	...
	switch (_menuID) do {
		...
		case MACRO_VARNAME_UI_ID_MAIN: {
			...
			// Create the controls
				...
				_ctrlLBCategories ctrlAddEventHandler ["LBSelChanged", {["ui_select_category", _this] call f_fnc_zeusUI}];
				...
				_ctrlLBUnits ctrlAddEventHandler ["LBSelChanged", {["ui_select_unit", _this] call f_fnc_zeusUI}];
				...
				_ctrlButtonSpawn ctrlAddEventHandler ["ButtonClick", {["ui_spawn", _this] call f_fnc_zeusUI}];
			...
			_ctrlLBPresets ctrlAddEventHandler ["LBSelChanged", {["ui_select_preset", _this] call f_fnc_zeusUI}];
		...
		case MACRO_VARNAME_UI_ID_PRESETS: {
			...
			if (isNull _zeusUI_presetsCtrlGrp) then {
				...
				_ctrlButtonExit ctrlAddEventHandler ["ButtonClick", {["ui_close"] spawn f_fnc_zeusUI}];	// SPAWN! Not call! Otherwise the game will crash! (gg BI)
				...
				{
					_x ctrlAddEventHandler ["CheckedChanged", {["ui_checkbox_changed", _this] call f_fnc_zeusUI}];
				} forEach [
					_ctrlCB_GAI,
				...
				["ui_checkbox_changed", [_ctrlCB_GAI,			missionNamespace getVariable [MACRO_VARNAME_PRESET_GAI, false]]] call f_fnc_zeusUI;
				...
				["ui_textbox_changed", [_ctrlTextBox_GAI_maxApproachDistance, nil, nil, nil, nil,	missionNamespace getVariable [MACRO_VARNAME_PRESET_GAI_MAXAPPROACHDISTANCE, 1000]]] call f_fnc_zeusUI;
				["ui_textbox_changed", [_ctrlTextBox_GAI_maxSearchDuration, nil, nil, nil, nil,		missionNamespace getVariable [MACRO_VARNAME_PRESET_GAI_MAXSEARCHDURATION, 30]]] call f_fnc_zeusUI;
				["ui_textbox_changed", [_ctrlTextBox_GAI_searchAreaSize, nil, nil, nil, nil,		missionNamespace getVariable [MACRO_VARNAME_PRESET_GAI_SEARCHAREASIZE, 30]]] call f_fnc_zeusUI;
				["ui_textbox_changed", [_ctrlTextBox_SAI_suppressionMul, nil, nil, nil, nil,		missionNamespace getVariable [MACRO_VARNAME_PRESET_SAI_SUPPRESSIONMUL, 1]]] call f_fnc_zeusUI;
				["ui_textbox_changed", [_ctrlTextBox_SAI_suppressionDurationMul, nil, nil, nil, nil,	missionNamespace getVariable [MACRO_VARNAME_PRESET_SAI_SUPPRESSIONDURATIONMUL, 1]]] call f_fnc_zeusUI;
