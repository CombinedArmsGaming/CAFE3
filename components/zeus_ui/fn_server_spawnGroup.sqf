/* --------------------------------------------------------------------------------------------------------------------
	...
	Arguments:
		...
		8:	<ARRAY>		(optional) Array of reinforcement units
		9:	<NUMBER>	(optional) Direction to face spawned units towards
		10:	<CODE>		(optional) A function to run upon the spawned units.  Takes params of: [["_group", grpNull], ["_vehicle", objNull]];
	Returns:
		(nothing)
-------------------------------------------------------------------------------------------------------------------- */
...
// Fetch our params
params ["_roles", "_pos", "_gear", "_side", ["_vehicleClass", ""], ["_enableAdvancedAI", false], ["_guerrillaAI", false], ["_suppressiveAI", false], ["_reinforcements", []], ["_dir", 0], ["_runAfter", f_fnc_addToCurator]];
...
{
	_args = [_roles, _pos, _vehicleClass, _gear, _side, _suppressiveAI, _guerrillaAI, _enableAdvancedAI, _runAfter, _reinforcements, _dir];
...
{
	_args = [_roles, _pos, _gear, _side, _suppressiveAI, _guerrillaAI, _enableAdvancedAI, _runAfter];
