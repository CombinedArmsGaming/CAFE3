...
if !(typeName _rifleArray == "ARRAY" and {count _rifleArray > 1 and {typeName (_rifleArray select 0) == "STRING"}}) exitWith
{
    DEBUG_FORMAT3_LOG("[GEARSCRIPT-2]: Unable to add rifle from loadout '%1' into crate %2 for side %3: the base variant doesn't have a rifle.",_loadout,_crateName,_side)
};


_rifleName = _rifleArray select 0;

_itemEntry = [_rifleName, _amount];
