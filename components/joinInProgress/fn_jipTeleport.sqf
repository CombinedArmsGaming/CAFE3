...
// BUB 2018-11-02 TODO :: jesus christ this
				...
				if (alive _x) then
				{
					if (_x != player) then
					{
						if (_x distance player > 50) then
						{
							...
							_bguy = _x;
							{
								if (((side (group _x)) getFriend (side (group player)) < 0.6 ) && alive _x) then
							...
							{ //Checks if the squad leader is in a vehicle
								if ((vehicle _x) emptyPositions "cargo" == 0) then
								{ //Checks if vehicle has empty seats
